import jobsJson from "@/data/jobs.json";
import profileJson from "@/data/profile.json";
import type { Job, SkillSummary } from "./types";

export const jobs = jobsJson as Job[];
export const profile = profileJson;

export function isExpired(job: Job, today = new Date()) {
  if (!job.closingDate) return false;
  const closing = new Date(job.closingDate + "T23:59:59");
  return closing.getTime() < today.getTime();
}

export function activeJobs(today = new Date()) {
  return jobs.filter((job) => !isExpired(job, today));
}

export function getSkillSummary(input = activeJobs()): SkillSummary[] {
  const counts = new Map<string, number>();
  for (const job of input) {
    for (const skill of [...job.skills, ...job.desirableSkills]) {
      counts.set(skill, (counts.get(skill) ?? 0) + 1);
    }
  }
  return [...counts.entries()]
    .map(([skill, count]) => ({ skill, count }))
    .sort((a, b) => b.count - a.count || a.skill.localeCompare(b.skill));
}

export function getMatchCounts() {
  return activeJobs().reduce(
    (acc, job) => {
      acc[job.matchBand] += 1;
      return acc;
    },
    { "Strong Match": 0, Reach: 0, "Development Target": 0 } as Record<Job["matchBand"], number>,
  );
}
